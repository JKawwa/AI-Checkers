...
class CheckersState(search_engine.TwoPlayerGameState):
    ...
    def is_end_state(self):
        ...
        board_winner = self.__board.get_winner()
        stale_mate = False
        state = self.get_parent()
        while state is not None:
            if state.get_hashable_state() == self.get_hashable_state():
                stale_mate = True
            state = state.get_parent()
        
        return (board_winner is not None or (not successors) or stale_mate)
    ...
    def get_winner(self):
        ...
        board_winner = self.__board.get_winner()
        if not successors:
            lock_winner = self.get_controller2() if self.get_max_turn() else self.get_controller1()
        return board_winner.get_controller() if board_winner else lock_winner if not successors else None
...
class Board:
    ...
    Args:
        ...
        state (Optional[CheckersState]): The state to set the board to.
    """

    # might want to make different board sizes
    width = 8 #: int: Width of the checkers board.
    height = 8 #: int: Height of the checkers board.
    ...
    def get_current_player(self):
        ...
        Returns:
            CheckersPlayer: the player who's turn it is.
        """
    ...
    def get_winner(self):
        ...
        Returns:
            CheckersPlayer: The winning player.
        """
    ...
    def jumpMove(self, piece, coordinate, action = None):
        ...
        Args:
           ...
           action (str): The action string to build up recursively. Don't provide this argument.
        Return: 
    ...
    def move(self, piece, coordinate):
        ...
        .. deprecated:: 0.1
            Use :meth:`Board.jumpMove` and :meth:`Board.regMove` instead.
...
class Piece:
    ...
    Args:
        ...
        direction(int) : 0 for up, 1 for down
        position (Optional[Position]): The position that the piece is on.
        piece (Optional[Piece]): A piece to copy properties from.
    """

    up = 0 #: int: A constant to define the UP direction.
    down = 1 #: int: A constant to define the DOWN direction.
    ...
    def get_player(self):
        """
        Gets the :class:'CheckersPlayer' that the piece is associated with.
        
        Returns:
            CheckersPlayer: The associated player.
    ...
    def get_moves(self):
        """
        Provides the directions the piece can move in.
